...
import { ensureCtx, ctxTime, preload, preloadDrums, setMix, scheduleStrum, scheduleBass, scheduleDrum, scheduleLead, schedulePiano, stopAll, voicingMidis, STRING_MIDI, AUDIO_DEFAULTS, setAudioSettings, setGuitarSet as applyGuitarSet, setBassSet as applyBassSet } from './audio.js';
...
const STRUMS = {
  ...
  travis: { label:'Travis', p:[ // thumb on every beat, soft finger picks on the ands
  ...
  funk: { label:'Funk', p:[ // straight-16th scratch accents; falls apart charmingly under shuffle
...
const GENRES = [
  ...
  { key:'folk',      label:'Folk',             tempo:96,  feel:'straight', strum:'travis',    drums:'off',   bass:'root5',  lead:'fills', drumFills:false, keys:'off' },
  ...
  { key:'pop',       label:'Pop',              tempo:116, feel:'straight', strum:'pop',       drums:'kit',   bass:'root5',  lead:'off',   drumFills:true,  keys:'on',  bassInst:'electric' },
  { key:'indiepop',  label:'Indie Pop',        tempo:128, feel:'straight', strum:'pop',       drums:'kit',   bass:'root',   lead:'off',   drumFills:false, keys:'off', bassInst:'electric' },
  { key:'bossa',     label:'Bossa Nova',       tempo:120, feel:'straight', strum:'bossa',     drums:'bossa', bass:'bossa',  lead:'off',   drumFills:false, keys:'on',  guitarInst:'nylon' },
  { key:'jazz',      label:'Jazz',             tempo:132, feel:'shuffle',  strum:'lofi',      drums:'swing', bass:'walk',   lead:'fills', drumFills:false, keys:'on',  guitarInst:'jazz' },
  { key:'funk',      label:'Funk',             tempo:102, feel:'straight', strum:'funk',      drums:'funk',  bass:'funk',   lead:'off',   drumFills:true,  keys:'off', bassInst:'electric', guitarInst:'muted' },
...
export default function Player() {
  ...
  // During the last bar of a climb pass, preview where we're headed: bar 1
  // flips to the next position's voicing with a pulse so the jump is telegraphed.
  const nextClimb=useMemo(()=>{
    if (!playing||posMode!=='climb'||passOrder.length<2||currentBar==null) return null;
    if (flat.playLen<2||currentBar!==flat.playLen-1) return null;
    ...
    if (na===passOrder[lp]) return null;
    return {anchor:na,path:pathForAnchor(na)};
  },[playing,posMode,passOrder,currentBar,flat.playLen,livePass,loop,pathForAnchor]);
  ...
  const buildSchedule=useCallback(()=>{
    ...
    for (let i=0;i<passBars;i++) {
      ...
      if (drums==='stomp') {
        ...
        [0,2].forEach(b=>events.push({t:base+b*spb,type:'drum',kind:'kick',g:0.75}));
        [0,0.5,1,1.5,2,2.5,3,3.5].forEach(b=>events.push({t:base+b*spb,type:'drum',kind:'hat',g:b%1?0.4:0.55}));
        [1,2.5].forEach(b=>events.push({t:base+b*spb,type:'drum',kind:'rim',g:0.9}));
      } else if (drums==='swing') {
        [0,1,1.5,2,3,3.5].forEach(b=>events.push({t:base+sw(b)*spb,type:'drum',kind:'ride',g:b%1?0.6:(b%2?1:0.8)}));
        [1,3].forEach(b=>events.push({t:base+b*spb,type:'drum',kind:'hat',g:0.5}));
        [0,2].forEach(b=>events.push({t:base+b*spb,type:'drum',kind:'kick',g:0.3}));
      } else if (drums==='funk') {
        [[0,1],[1.75,0.8],[2.5,0.9]].forEach(([b,g])=>events.push({t:base+b*spb,type:'drum',kind:'kick',g}));
        [1,3].forEach(b=>events.push({t:base+b*spb,type:'drum',kind:'snare',g:1}));
        [0.75,2.25].forEach(b=>events.push({t:base+b*spb,type:'drum',kind:'snare',g:0.4}));
        [0,0.5,1,1.5,2,2.5,3,3.5].forEach(b=>events.push({t:base+b*spb,type:'drum',kind:'hat',g:b%1?0.55:0.85}));
      }
  ...
  },[flat,chords,tempo,feel,strum,drums,drumFills,bassMode,lead,keys,passOrder,pathForAnchor,grips,sound,guitarSet,bassSet]);
  ...
  const save=()=>{
    ...
    if (!name) return;
    const entry={name,key,tempo,sections,arrangement,pins,feel,strum,drums,drumFills,bassMode,lead,keys,genre};
    const next=[...saved.filter(s=>s.name!==name),entry];
    setSaved(next);
    localStorage.setItem(STORE_KEY,JSON.stringify(next));
  };
  const loadEntry=s=>{
    setKey(s.key); setTempo(s.tempo); setFeel(s.feel||'straight');
    if (s.sections) {
  ...
  return (
    ...
    <div className="max-w-4xl mx-auto p-4">
      ...
      <div className="mb-4 flex flex-wrap gap-x-8 gap-y-3">
        ...
        {view==='triads'&&(
          <div>
            <label className="text-xs text-gray-400 uppercase tracking-wide mb-2 block">Position <span className="normal-case text-gray-600">{positions.length>1?`${displayAnchor+1}/${positions.length} · `:''}{posLabel}</span>{nextClimb&&<span className="normal-case text-amber-400 animate-pulse"> → {nextClimb.anchor+1}</span>}</label>
            <div className="flex gap-1.5">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {[['off','Off'],['stomp','Stomp'],['kit','Kit'],['train','Train'],['bossa','Bossa'],['swing','Swing'],['funk','Funk']].map(([k,l])=>(
        ...
        <div className="flex items-center gap-1.5">
          ...
          {[['off','Off'],['root','Root'],['root5','Root–5th'],['walk','Walking'],['boogie','Boogie'],['bossa','Bossa'],['funk','Funk']].map(([k,l])=>(
              ...
              <div key={ch} className="flex flex-wrap items-center gap-x-4 gap-y-1">
                ...
                {ch==='guitar'&&(
                  <span className="flex items-center gap-1">
                    ...
                    {[['musyng','Musyng'],['fluid','Fluid'],['fatboy','FatBoy'],['nylon','Nylon'],['jazz','E.Jazz'],['muted','E.Muted']].map(([k,l])=>(
      ...
      <div className="bg-gray-900 rounded-xl p-4 border border-gray-800 mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {bars.map((bar,i)=>{
            ...
            const preview=view==='triads'&&fi===0&&nextClimb?.path[0]?nextClimb:null;
            const path=preview?preview.path:triadPath;
            return (
              <div key={i} onClick={()=>setEditIdx(isEdit?null:i)}
                   className={`cursor-pointer rounded-lg border px-2 pt-1.5 pb-2 flex flex-col items-center transition-all min-w-[110px] ${preview?'border-amber-400 ring-1 ring-amber-400/70 animate-pulse bg-gray-950':isCur?'border-amber-500 bg-amber-500/10':isEdit?'border-emerald-500 bg-emerald-500/10':'border-gray-800 bg-gray-950 hover:border-gray-600'}`}>
                <div className="text-[10px] text-gray-600 self-start">bar {i+1}{preview?<span className="text-amber-400"> · next: {fretWindow(preview.path[0])}</span>:<>{view==='triads'&&auto&&path[fi]&&<span className="text-gray-500"> · {path[fi].set.label}</span>}{pins[`${activeSec}:${i}`]!=null&&<span className="text-emerald-500"> · pinned</span>}</>}</div>
                ...
                {view==='cowboy'
                  ? (grips[fi]
                      ...
                      : (path[fi]?<FretDiag voicing={path[fi]} strs={path[fi].set.strs} name={null} root={ch.root} size="small"/>:<div className="text-xs text-gray-600 italic p-4">no grip</div>))
                  : (path[fi]
                      ? <FretDiag voicing={path[fi]} strs={path[fi].set.strs} name={null} root={ch.root} size="small"/>
