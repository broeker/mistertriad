...
import { buildSchedule as buildScheduleFn, transposeSchedule, centerOf, posCost, pinKeyOf, pinMatches, resolvePin } from './arranger.js';
...
export default function Player() {
  ...
  const [capo,setCapo]=useState(0); // 0-7 semitones; the key picker stays the sounding key
  ...
  // Capo: shapes come from shapeKey (what the hand fingers), sound from key
  // (what the band plays back at) — buildSchedule transposes shape → sound.
  const shapeKey=((key-capo)%12+12)%12;
  ...
  const chords=useMemo(()=>flat.bars.map(b=>chordOf(b,shapeKey)),[flat,shapeKey]);
  ...
  const buildSchedule=useCallback(()=>transposeSchedule(buildScheduleFn({
  ...
  }),capo),[flat,chords,meter,tempo,feel,strum,drums,drumFills,bassMode,bassFills,guitarFills,backup,lead,leadEvery,keys,passOrder,passPath,grips,sound,guitarSet,bassSet,pianoSet,backupSet,capo]);
  ...
  const save=()=>{
    ...
    if (!name) return;
    const entry={name,key,capo,meter,tempo,sections,arrangement,pins,rolls,selectedSets,feel,strum,drums,drumFills,bassMode,bassFills,guitarFills,backup,lead,leadEvery,keys,genre};
    setSaved([...saved.filter(s=>s.name!==name),entry]);
  };
  const loadEntry=s=>{
    const m=s.meter&&METERS[s.meter]?s.meter:'4/4'; // legacy saves are 4/4
    setKey(s.key); setCapo(s.capo||0); setMeter(m); setTempo(s.tempo);
  ...
  return (
    ...
    <div className="max-w-4xl mx-auto p-4">
      ...
      <button onClick={()=>setSongOpen(o=>!o)} className="w-full flex items-center gap-3 px-4 py-2.5 text-left">
        ...
        {!songOpen&&<span className="text-xs text-gray-600 truncate">{NOTES[key]}{capo?` capo ${capo}`:''} · {GENRES.find(g=>g.key===genre)?.label} · {(PROGRESSIONS[genre]||[]).find(p=>barsKey(bars)===barsKey(toBars(p.bars)))?.name||`${bars.length} bars`}</span>}
      ...
      <div className="mb-4">
        <label className="text-xs text-gray-400 uppercase tracking-wide mb-2 block">Capo{capo>0&&<span className="normal-case text-gray-600"> — {NOTES[shapeKey]} shapes · sounds {NOTES[key]}</span>}</label>
        <div className="flex flex-wrap gap-1.5">
          {[0,1,2,3,4,5,6,7].map(c=>(<button key={c} onClick={()=>setCapo(c)} className={btn(capo===c)}>{c||'None'}</button>))}
        </div>
      </div>
      ...
      <div className="bg-gray-900 rounded-xl p-4 border border-gray-800 mb-4">
        ...
        {editIdx!==null&&editIdx<bars.length&&(
          <div className="mt-4 p-3 bg-gray-800 rounded-lg border border-emerald-700/50">
            ...
            <div className="text-xs text-gray-500 mb-1.5">Chord (degree in {NOTES[key]}{capo>0&&`, ${NOTES[shapeKey]} shapes`})</div>
